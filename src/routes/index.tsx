...
function Index() {
  return (
    ...
    <section className="system-stage"><div className="site-container grid items-center gap-14 py-20 md:grid-cols-[0.8fr_1.2fr] md:py-28"><div><p className="eyebrow text-signal">Systematisch vorgehen</p><h2 className="section-title text-primary-foreground">Nicht bei der Lösung anfangen. Beim Verstehen.</h2><p className="mt-6 max-w-lg leading-7 text-primary-foreground/65">Wir ordnen Anforderungen, machen Zusammenhänge sichtbar und schaffen eine technische Grundlage, die mit Ihrem Unternehmen wachsen kann.</p><a href="/leistungen" className="text-link mt-8">Leistungen ansehen <ArrowUpRight size={17} /></a></div><div className="architecture" aria-label="Darstellung unseres Vorgehens"><div className="arch-core">softex<span>.solutions</span></div>{["Analyse","Architektur","Entwicklung","Einführung"].map((x,i)=><div key={x} className={`arch-node node-${i+1}`}><small>0{i+1}</small>{x}</div>)}<svg viewBox="0 0 600 420" aria-hidden="true"><path d="M118 94 L300 210 L488 88 M300 210 L490 330 M300 210 L105 330" /></svg></div></div></section>
